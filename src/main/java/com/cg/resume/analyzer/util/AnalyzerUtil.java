...
import com.cg.resume.analyzer.model.ProfessionalDetailsModel;
import com.cg.resume.analyzer.model.SkillDetailsModel;
...
public final class AnalyzerUtil {
	...
	public static Map<String, Object> retriveProfileDetailsFromTable(DocxDetailsModel docxDetails) {
		...
		for (XWPFParagraph paragrph : para) {
			System.out.println("para" + paragrph.getParagraphText());
		}
	...
	public static Map<String, Object> retriveProfecionalDetailsFromTable(DocxDetailsModel docxDetails) {
		...
		ProfessionalDetailsModel ProfessionalDetails = new ProfessionalDetailsModel();
		for (XWPFTable table : docxDetails.getTableList()) {
			for (XWPFTableRow row : table.getRows()) {
	...
	public static Map<String, Object> retriveProjectdetailsFromTable(DocxDetailsModel docxDetails) {
		...
		for (XWPFTable table : docxDetails.getTableList()) {
			for (XWPFTableRow row : table.getRows()) {
				// System.out.println("..."+row.getCell(0).getText().trim());
				/* this code throws null pointer exception
				 * if(!(row.getCell(1).getText().equals(""))) {
				 * if(AnalyzerConsts.projectList.contains(row.getCell(1).getText().trim())) {
				 * if(row.getCell(0).getText().trim()!= null) { project.put("Project Duration",
				 * row.getCell(1).getText().trim()); } else {
				 * project.put(row.getCell(1).getText().trim(),
